...
from datasets.datautils3d import load_with_coord , load_diffusion_coords_idensity , load_diffusion_random_blocks , load_diffusion_overlap_blocks
...
def main():
    ...
    for epoch in range(first_epoch, cfg.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            clean_images = batch['value'].to(weight_dtype)
            print(" clean_image :" , " min :" , clean_images.min() ,"max", clean_images.max() , "std" , clean_images.std() )
            coords = batch['coords'].to(weight_dtype)
            # Sample noise that we'll add to the images
            # noise = torch.randn(clean_images[:,3:4,...].shape,
            ...
            #noisy_images = torch.cat([clean_images[:,:3,...], noisy_idensity] , dim=1)
            noisy_images = torch.cat([coords, noisy_idensity] , dim=1)
            #pdb.set_trace()
